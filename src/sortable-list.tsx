...
import { Row } from "./row";
import { getTranslateY, Key, Nullable, RowCreator } from "./shared";
...
const InnerSortableList = <Row extends HTMLElement, Id extends Key>(
	props: Props<Row, Id>,
) => {
	...
	const [currentDraggedIndexState, setCurrentDraggedIndexState] =
		React.useState<number>();
	const [draggingItemIdState, setDraggingItemId] = React.useState<Key>();
	const rowRefs = React.useRef<React.RefObject<Row | null>[]>([]);
	const upDiffRef = React.useRef<number>(undefined);
	const downDiffRef = React.useRef<number>(undefined);

	const idToIndex = React.useMemo(
		() =>
			props.ids.reduce(
				(previous, current, i) => ({ ...previous, [current]: i }),
	...
	const setIds = React.useCallback(() => {
		if (
			draggingItemIdState == undefined ||
			currentDraggedIndexState == undefined
		)
			return;
	...
	const onStartDragging = React.useCallback(
		(id: Key) => {
			...
			const draggingRect =
				rowRefs.current[index].current?.getBoundingClientRect();
			if (draggingRect == undefined) return;

			const upRect =
				rowRefs.current[index - 1]?.current?.getBoundingClientRect();
			const downRect =
				rowRefs.current[index + 1]?.current?.getBoundingClientRect();
			if (upRect != undefined)
				upDiffRef.current = draggingRect.bottom - upRect.bottom;
			if (downRect != undefined)
				downDiffRef.current = draggingRect.top - downRect.top;
		},
		[idToIndex],
	);
	...
	const onDrag = React.useCallback(
		(id: Key) => {
			...
			const draggingDOMPosition = getDOMPosition(
				rowRefs.current[index].current,
			);
			if (draggingDOMPosition == undefined) return;
			startAutoScrollingIfNeeded(rowRefs.current[index].current!);

			const [upIndex, downIndex] = (() => {
				if (currentDraggedIndexState == undefined)
					return [index - 1, index + 1];
				if (currentDraggedIndexState < index)
					return [currentDraggedIndexState - 1, currentDraggedIndexState];
				if (currentDraggedIndexState > index)
					return [currentDraggedIndexState, currentDraggedIndexState + 1];

				return [index - 1, index + 1];
			})();

			const upDOMPosition = getDOMPosition(rowRefs.current[upIndex]?.current);
			const downDOMPosition = getDOMPosition(
				rowRefs.current[downIndex]?.current,
			);

			if (
				upDOMPosition != undefined &&
				draggingDOMPosition.top <= upDOMPosition.top
			) {
				setCurrentDraggedIndexState((currentDraggedIndexState ?? index) - 1);
			}
			if (
				downDOMPosition != undefined &&
				draggingDOMPosition.bottom >= downDOMPosition.bottom
			) {
				setCurrentDraggedIndexState((currentDraggedIndexState ?? index) + 1);
			}
		},
		[
			startAutoScrollingIfNeeded,
			idToIndex,
...
export const SortableList = <Row extends HTMLElement, Id extends Key>(
	props: Props<Row, Id>,
) => (
	<AutoScrollerValueContextProvider>
